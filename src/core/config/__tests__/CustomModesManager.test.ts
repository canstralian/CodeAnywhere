...
describe("CustomModesManager", () => {
	...
	const mockRoomodes = `${path.sep}mock${path.sep}workspace${path.sep}.codefluxmodes`
	...
	describe("getCustomModes", () => {
		it("should merge modes with .codefluxmodes taking precedence", async () => {
			const settingsModes = [
			...
			// mode2 should come from .codefluxmodes since it takes precedence
		...
		it("should handle missing .codefluxmodes file", async () => {
		...
		it("should handle invalid JSON in .codefluxmodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .codefluxmodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .codefluxmodes precedence", async () => {
			...
			// Should update global state with merged modes where .codefluxmodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .codefluxmodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .codefluxmodes file when adding project-specific mode", async () => {
			...
			// Mock .codefluxmodes to not exist initially
			...
			// Verify .codefluxmodes was created with the project mode
			...
			// Verify the content written to .codefluxmodes
