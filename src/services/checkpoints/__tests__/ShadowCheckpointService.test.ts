...
const initWorkspaceRepo = async ({
	workspaceDir,
	userName = "CodeFlux AI Kit",
	userEmail = "support@codeflux.dev",
...
describe.each([
	...
	describe(`${klass.name}#renameNestedGitRepos`, () => {
		it("handles nested git repositories during initialization", async () => {
			...
			await mainGit.init()
			await mainGit.addConfig("user.name", "CodeFlux AI Kit")
			await mainGit.addConfig("user.email", "support@codeflux.dev")
			...
			await nestedGit.addConfig("user.name", "CodeFlux AI Kit")
			await nestedGit.addConfig("user.email", "support@codeflux.dev")
...
describe("ShadowCheckpointService", () => {
	...
	describe("getTaskStorage", () => {
		...
		it("returns undefined when workspace repo exists but has no task branch", async () => {
			...
			await git.init()
			await git.addConfig("user.name", "CodeFlux AI Kit")
