...
const ROOMODES_FILENAME = ".codefluxmodes"
...
export class CustomModesManager {
	...
	private async watchCustomModesFiles(): Promise<void> {
		...
		this.disposables.push(
			vscode.workspace.onDidSaveTextDocument(async (document) => {
				if (arePathsEqual(document.uri.fsPath, settingsPath)) {
					...
					// Get modes from .codefluxmodes if it exists (takes precedence)
					const roomodesPath = await this.getWorkspaceRoomodes()
					const roomodesModes = roomodesPath ? await this.loadModesFromFile(roomodesPath) : []

					// Merge modes from both sources (.codefluxmodes takes precedence)
		...
		// Watch .codefluxmodes file if it exists
		const roomodesPath = await this.getWorkspaceRoomodes()
		if (roomodesPath) {
			this.disposables.push(
				vscode.workspace.onDidSaveTextDocument(async (document) => {
					if (arePathsEqual(document.uri.fsPath, roomodesPath)) {
						...
						// .codefluxmodes takes precedence
	...
	async getCustomModes(): Promise<ModeConfig[]> {
		...
		// Get modes from .codefluxmodes if it exists
		const roomodesPath = await this.getWorkspaceRoomodes()
		const roomodesModes = roomodesPath ? await this.loadModesFromFile(roomodesPath) : []
