...
export class Cline extends EventEmitter<ClineEvents> {
	...
	async say(
		...
		if (this.abort) {
			throw new Error(`[CodeFlux AI Kitine#say] task ${this.taskId}.${this.instanceId} aborted`)
		}
	...
	async sayAndCreateMissingParamError(toolName: ToolUseName, paramName: string, relPath?: string) {
		await this.say(
			"error",
			`CodeFlux AI Kit tried to use ${toolName}${
		...
		if (this.consecutiveMistakeCount >= 3) {
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "CodeFlux AI Kit uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.7 Sonnet for its advanced agentic coding capabilities.",
			)
